...
// /login included).
// Reads Open Secret's token key straight from local storage: the web app owns
// the storage it hands the SDK, so the storage backend is known here.
...
export const authQueryOptions = () =>
  queryOptions({
    ...
    queryFn: async (): Promise<AuthState> => {
      ...
      // session restore completes, by reading Open Secret's token key straight
      // from local storage (the web app owns the storage it hands the SDK).
 ...
 */
export const useHandleSessionEvents = ({
  onSessionExpired,
}: {
  ...
  useEffect(() => {
    const handleSessionExpired = () => {
      ...
      void endSessionCleanup().catch((error) => {
        // The SDK has already ended the session, so nothing sensitive
        // survives a failed web-side cleanup — the worst case is stale
        // logged-in UI until the user reloads (PR #1166 review).
