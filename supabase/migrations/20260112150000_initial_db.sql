...
-- Needed for pg_cron. See https://supabase.com/docs/guides/cron/install?queryGroups=database-method&database-method=sql
...
-- Disable pg_graphql extension which Supabase adds by default
...
create type "wallet"."currency" as enum ('BTC', 'USD');

create type "wallet"."receive_quote_type" as enum ('LIGHTNING', 'CASHU_TOKEN');
...
create table if not exists "wallet"."users" (
  ...
  "default_currency" "wallet"."currency" default 'BTC' not null,
  ...
  constraint "users_default_currency_has_account" check (((("default_currency" = 'BTC') and ("default_btc_account_id" is not null)) or (("default_currency" = 'USD') and ("default_usd_account_id" is not null)))),
  constraint "users_username_format" check (("username" ~ '^[a-z0-9_-]+$'::"text")),
  constraint "users_username_length" check ((("length"("username") >= 5) and ("length"("username") <= 20)))
);
...
-- Table: accounts
create type "wallet"."account_type" as enum ('cashu', 'spark');
create type "wallet"."account_purpose" as enum ('transactional', 'gift-card');
...
create table if not exists "wallet"."accounts" (
  ...
  "name" "text" not null,
  "type" "wallet"."account_type" not null,
  "purpose" "wallet"."account_purpose" default 'transactional' not null,
  "currency" "wallet"."currency" not null,
...
-- Indexes
create unique index "cashu_accounts_user_currency_mint_url_unique" on "wallet"."accounts" using "btree" ("user_id", "currency", (("details" ->> 'mint_url'::"text"))) where ("type" = 'cashu');
...
create or replace trigger "enforce_max_accounts_limit" before insert on "wallet"."accounts" for each row execute function "wallet"."enforce_accounts_limit"();
...
begin
  ...
  if v_contact_count >= 200 then
    raise exception 
      using
        hint = 'LIMIT_REACHED',
        message = 'Maximum number of contacts limit reached.',
        detail = format('Contacts count: %s, limit: 200.', v_contact_count);
  end if;
...
-- Table: transactions
create type "wallet"."transaction_direction" as enum ('SEND', 'RECEIVE');
create type "wallet"."transaction_type" as enum ('CASHU_LIGHTNING', 'CASHU_TOKEN', 'SPARK_LIGHTNING');
create type "wallet"."transaction_state" as enum ('DRAFT', 'PENDING', 'COMPLETED', 'FAILED', 'REVERSED');
create type "wallet"."acknowledgment_status" as enum ('pending', 'acknowledged');
...
create table if not exists "wallet"."transactions" (
  ...
  "state" "wallet"."transaction_state" not null,
  ...
  "acknowledgment_status" "wallet"."acknowledgment_status",
...
create index "idx_transactions_spark_transfer_id" on "wallet"."transactions" using "btree" ((("transaction_details" ->> 'sparkTransferId'::"text"))) where (("type" = 'SPARK_LIGHTNING') and (("transaction_details" ->> 'sparkTransferId'::"text") is not null));
create index "idx_transactions_user_acknowledgment_pending" on "wallet"."transactions" using "btree" ("user_id", "acknowledgment_status") where ("acknowledgment_status" = 'pending');
create index "idx_transactions_user_state_sort_order_created_at_id" on "wallet"."transactions" using "btree" ("user_id", "state_sort_order" desc, "created_at" desc, "id" desc) where ("state" = any (array['PENDING', 'COMPLETED', 'REVERSED']::"wallet"."transaction_state"[]));
create index "idx_transactions_user_account_id_state_sort_order_created_at_id" on "wallet"."transactions" using "btree" ("user_id", "account_id", "state_sort_order" desc, "created_at" desc, "id" desc) where ("state" = any (array['PENDING', 'COMPLETED', 'REVERSED']::"wallet"."transaction_state"[]));
...
-- Table: cashu_proofs
create type "wallet"."cashu_proof_state" as enum ('UNSPENT', 'RESERVED', 'SPENT');
...
create table if not exists "wallet"."cashu_proofs" (
  ...
  "dleq" "jsonb",
  "witness" "jsonb",
  "state" "wallet"."cashu_proof_state" default 'UNSPENT' not null,
  "version" integer default 0 not null,
  "created_at" timestamp with time zone default "now"() not null,
  ...
  constraint "cashu_proofs_version_check" check (("version" >= 0))
);
...
create index "idx_cashu_proofs_state_spent_at" on "wallet"."cashu_proofs" using "btree" ("state", "spent_at") where ("state" = 'SPENT');
...
create type "wallet"."cashu_receive_quote_state" as enum ('UNPAID', 'EXPIRED', 'PAID', 'COMPLETED', 'FAILED');
...
create table if not exists "wallet"."cashu_receive_quotes" (
  ...
  "state" "wallet"."cashu_receive_quote_state" not null,
  "keyset_id" "text",
  "keyset_counter" integer,
  "version" integer default 0 not null,
  "transaction_id" "uuid" not null references "wallet"."transactions"("id"),
  "type" "wallet"."receive_quote_type" not null,
  ...
  constraint "cashu_receive_quotes_cashu_token_melt_initiated_check" check ((("type" <> 'CASHU_TOKEN') or ("cashu_token_melt_initiated" is not null))),
...
create type "wallet"."cashu_receive_swap_state" as enum ('PENDING', 'COMPLETED', 'FAILED');
...
create table if not exists "wallet"."cashu_receive_swaps" (
  ...
  "state" "wallet"."cashu_receive_swap_state" default 'PENDING' not null,
  "version" integer default 0 not null,
  "failure_reason" "text",
  "transaction_id" "uuid" not null references "wallet"."transactions"("id"),
  "encrypted_data" "text" not null,
  constraint "cashu_receive_swaps_keyset_counter_check" check (("keyset_counter" >= 0)),
  constraint "cashu_receive_swaps_version_check" check (("version" >= 0)),
...
create type "wallet"."cashu_send_quote_state" as enum ('UNPAID', 'PENDING', 'EXPIRED', 'FAILED', 'PAID');
...
create table if not exists "wallet"."cashu_send_quotes" (
  ...
  "user_id" "uuid" not null references "wallet"."users"("id"),
  "account_id" "uuid" not null references "wallet"."accounts"("id"),
  "currency_requested" "wallet"."currency" not null,
  "keyset_id" "text" not null,
  "keyset_counter" integer not null,
  "number_of_change_outputs" integer not null,
  "state" "wallet"."cashu_send_quote_state" default 'UNPAID' not null,
  "failure_reason" "text",
  "version" integer default 0 not null,
  "transaction_id" "uuid" not null references "wallet"."transactions"("id"),
  "encrypted_data" "text" not null,
  "payment_hash" "text" not null,
  "quote_id_hash" "text" not null,
  constraint "cashu_send_quotes_keyset_counter_check" check (("keyset_counter" >= 0)),
  constraint "cashu_send_quotes_number_of_change_outputs_check" check (("number_of_change_outputs" >= 0)),
  constraint "cashu_send_quotes_version_check" check (("version" >= 0))
);

-- Indexes
create unique index "cashu_send_quotes_quote_id_hash_key" on "wallet"."cashu_send_quotes" using "btree" ("account_id", "quote_id_hash") where (("quote_id_hash" is not null) and ("state" <> 'FAILED'));
...
create type "wallet"."cashu_send_swap_state" as enum ('DRAFT', 'PENDING', 'COMPLETED', 'FAILED', 'REVERSED');
...
create table if not exists "wallet"."cashu_send_swaps" (
  ...
  "keyset_counter" integer,
  "token_hash" "text",
  "state" "wallet"."cashu_send_swap_state" not null,
  "version" integer default 0 not null,
  "created_at" timestamp with time zone default "now"() not null,
  "failure_reason" "text",
  "encrypted_data" "text" not null,
  "requires_input_proofs_swap" boolean default false not null,
  constraint "cashu_send_swaps_keyset_counter_check" check (("keyset_counter" is null or "keyset_counter" >= 0)),
  constraint "cashu_send_swaps_token_hash_required_check" check (((("state" = any (array['DRAFT', 'FAILED']::"wallet"."cashu_send_swap_state"[])) and ("token_hash" is null)) or (("state" <> all (array['DRAFT', 'FAILED']::"wallet"."cashu_send_swap_state"[])) and ("token_hash" is not null)))),
  constraint "cashu_send_swaps_version_check" check (("version" >= 0))
);

-- Indexes
create index "idx_cashu_send_swaps_state_created_at" on "wallet"."cashu_send_swaps" using "btree" ("state", "created_at") where ("state" = any (array['COMPLETED', 'FAILED', 'REVERSED']::"wallet"."cashu_send_swap_state"[]));
create index "idx_cashu_send_swaps_transaction_id" on "wallet"."cashu_send_swaps" using "btree" ("transaction_id");
create index "idx_cashu_send_swaps_user_id_state" on "wallet"."cashu_send_swaps" using "btree" ("user_id", "state") where ("state" = any (array['DRAFT', 'PENDING']::"wallet"."cashu_send_swap_state"[]));
...
create type "wallet"."spark_receive_quote_state" as enum ('UNPAID', 'EXPIRED', 'PAID', 'FAILED');

create table if not exists "wallet"."spark_receive_quotes" (
  "id" "uuid" default "gen_random_uuid"() not null primary key,
  "type" "wallet"."receive_quote_type" not null,
  "state" "wallet"."spark_receive_quote_state" default 'UNPAID' not null,
  "created_at" timestamp with time zone default "now"() not null,
  "expires_at" timestamp with time zone not null,
  "payment_hash" "text" not null,
  ...
  "failure_reason" "text",
  "cashu_token_melt_initiated" boolean,
  constraint "spark_receive_quotes_cashu_token_melt_initiated_check" check ((("type" <> 'CASHU_TOKEN') or ("cashu_token_melt_initiated" is not null))),
  constraint "spark_receive_quotes_version_check" check (("version" >= 0))
);
...
create type "wallet"."spark_send_quote_state" as enum ('UNPAID', 'PENDING', 'COMPLETED', 'FAILED');

create table if not exists "wallet"."spark_send_quotes" (
  "id" "uuid" default "gen_random_uuid"() not null primary key,
  "state" "wallet"."spark_send_quote_state" default 'UNPAID' not null,
  "created_at" timestamp with time zone default "now"() not null,
  "payment_hash" "text" not null,
  "spark_id" "text",
  "spark_transfer_id" "text",
  ...
  "encrypted_data" "text" not null,
  constraint "spark_send_quotes_spark_id_required" check ((("state" = 'UNPAID') or ("state" = 'FAILED') or ("spark_id" is not null))),
  constraint "spark_send_quotes_version_check" check (("version" >= 0))
);
...
create index "idx_spark_send_quotes_unresolved" on "wallet"."spark_send_quotes" using "btree" ("user_id", "state") where ("state" = any (array['UNPAID', 'PENDING']::"wallet"."spark_send_quote_state"[]));
create unique index "spark_send_quotes_payment_hash_active_unique" on "wallet"."spark_send_quotes" using "btree" ("user_id", "payment_hash") where ("state" = any (array['UNPAID', 'PENDING']::"wallet"."spark_send_quote_state"[]));
...
create or replace function "wallet"."add_cashu_proofs"(
  ...
  "p_user_id" "uuid",
  "p_account_id" "uuid",
  "p_proofs_state" "wallet"."cashu_proof_state" default 'UNSPENT',
...
create type "wallet"."account_input" as (
  ...
  "purpose" "wallet"."account_purpose",
...
declare
  v_swap wallet.cashu_send_swaps;
  v_reversal_transaction_state wallet.transaction_state;
  v_spent_proofs wallet.cashu_proofs[];
  v_account wallet.accounts;
  v_account_with_proofs jsonb;
begin
...
declare
  v_query text;
  v_cursor_null_count integer;
begin
  -- Validate cursor params: either all must be set or all must be null
  v_cursor_null_count := (p_cursor_state_sort_order is null)::int + (p_cursor_created_at is null)::int + (p_cursor_id is null)::int;
  if v_cursor_null_count not in (0, 3) then
    raise exception
      using
        hint = 'INVALID_ARGUMENT',
        message = 'Cursor parameters must be either all set or all null',
        detail = format('p_cursor_state_sort_order: %s, p_cursor_created_at: %s, p_cursor_id: %s', p_cursor_state_sort_order, p_cursor_created_at, p_cursor_id);
  end if;
  ...
  if p_account_id is not null then
    v_query := v_query || ' and t.account_id = $6';
  end if;

  -- Add cursor filter if provided
  if p_cursor_created_at is not null then
    v_query := v_query || ' and (t.state_sort_order, t.created_at, t.id) < ($2, $3, $4)';
  end if;

  -- Add ordering and limit
  v_query := v_query || ' order by t.state_sort_order desc, t.created_at desc, t.id desc limit $5';
