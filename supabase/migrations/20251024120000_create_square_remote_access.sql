...
create policy "Authenticated users cannot access square credentials"
  on mints.square_merchant_credentials
  for all
  to authenticated
  using (false);

create policy "Remote app roles can select their assigned merchant"
  on mints.square_merchant_credentials
  for select
  to public
  using (
...
create policy "Remote app roles can update their assigned merchant"
  on mints.square_merchant_credentials
  for update
  to public
  using (
...
create policy "Service role only can manage remote app access"
  on mints.square_remote_app_access
  for all
  to authenticated
  using (false);

create policy "Remote roles can see their own mapping"
  on mints.square_remote_app_access
  for select
  to public
