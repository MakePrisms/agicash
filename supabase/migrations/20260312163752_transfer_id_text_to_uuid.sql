-- Migration: Change p_transfer_id param type from text to uuid in all 4 quote
-- creation functions. The transferId is always generated via crypto.randomUUID()
-- so uuid is the correct type.

-- 1. Cashu receive quote
...
drop function if exists wallet.create_cashu_receive_quote(
  ...
  wallet.transaction_purpose, text
);
