-- Migration: Add deposit_fee field to cashu_receive_quotes table
-- Purpose: Add optional deposit_fee column to track the mint's deposit fee for receive quotes
-- Affected tables: wallet.cashu_receive_quotes
-- Affected functions: wallet.create_cashu_receive_quote

-- Add deposit_fee column to cashu_receive_quotes table
-- This column stores the optional fee that the mint charges to deposit money in
alter table wallet.cashu_receive_quotes
add column deposit_fee numeric default null;

comment on column wallet.cashu_receive_quotes.deposit_fee is 'Optional fee that the mint charges to deposit money in';
...
-- Update the create_cashu_receive_quote function to accept optional deposit_fee parameter
-- This is a copy of the latest version with the deposit_fee parameter added
create or replace function wallet.create_cashu_receive_quote(
    ...
    p_deposit_fee numeric default null
)
