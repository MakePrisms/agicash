...
CREATE OR REPLACE FUNCTION wallet.upsert_user_with_accounts(p_user_id uuid, p_email text, p_email_verified boolean, p_accounts jsonb[], p_cashu_locking_xpub text, p_encryption_public_key text, p_spark_public_key text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$declare
  result_user jsonb;
...
begin
  ...
  if not jsonb_path_exists(array_to_json(p_accounts)::jsonb, '$[*] ? (@.currency == "BTC" && @.type == "spark")') then
    raise exception 'At least one BTC Spark account is required';
  end if;
