...
- **(auth slice — the immediate next work item)** Auth deliberately stayed on OpenSecret (D13-1). Migrate the route guards (`_auth.tsx` `login_method`/`email`, `_protected.tsx` `shouldUserVerifyEmail`/`hasUserChanged`/`ensureUserData` bootstrap) off the OpenSecret identity; re-home `Sentry.setUser` + the session-hint cookie onto the bridge's `auth:signed-in/out` (currently forward-compat STUBS); flip `authQueryOptions`→`sdk.user.getCurrentUser` and retire `useAuthActions`→`sdk.auth.*`. **SDK prereqs:** add an `auth:session-expired` PRODUCER + an expiry scheduler (port `useHandleSessionExpiry`), and expose `login_method` (or prove it debug-only) + a cheap pre-bootstrap `isLoggedIn`/`getCurrentUserId` on the public `Sdk`. Delete `useHandleSessionExpiry` + `ensureUserData` + the web `entry.client.tsx` `configure()` (plan §8.5 Step 12, reassigned here) in the SAME change that flips the guard (dual-bootstrap / dual-expiry-handler / dual-OpenSecret-config hazard).
