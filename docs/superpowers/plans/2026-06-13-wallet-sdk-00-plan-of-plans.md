...
| 13b | auth-slice | guards off OpenSecret identity onto `sdk.auth`/`sdk.user` + bridge auth handlers + SDK prereqs (cheap `isLoggedIn`/`getCurrentUserId` + `auth:session-expired` producer/scheduler); delete `useHandleSessionExpiry` + `ensureUserData` + web `entry.client` `configure()` | ✅ [planned](2026-06-13-wallet-sdk-13b-auth-slice.md) (3 SDK + 4 web tasks; 6 forks resolved; NOT yet executed) |
| 14 | S14 | server routes (LN-address → `getServerSdk()`; delete `lightning-address-service.ts` + `database.server.ts`) | ✅ [planned](2026-06-13-wallet-sdk-14-server-routes.md) (1 codec + 3 route + delete + docs tasks; 8 forks resolved; NOT yet executed) |
