...
| 10 | S10 | `ServerSdk` facade over shared internals | ✅ [done](2026-06-13-wallet-sdk-10-server-sdk.md) (9 tasks, all reviewed clean, gate green — 624 SDK tests, 10 commits incl. plan doc); `createServer(config): ServerSdk` over shared internals (service-role supabase + dedicated server spark wallet + seedless cashu clients); 3 session-less LNURL ops (resolve username→capability / create receive quote / LUD-21 verify-settle); `encryptToPublicKey` exported; verify-token xchacha + LUD JSON wire format stay route-side (S14). **Phase 1 complete.** |
| 11 | S11 | web cut-over ENTRY POINTS (`getSdk()` client singleton + `SdkConfig` assembly + `createServer()` server instance) | ✅ [done](2026-06-13-wallet-sdk-11-web-entry-points.md) (4 tasks, all reviewed clean, gate green) — additive: `@agicash/wallet-sdk` workspace dep + barrel re-export of `MintBlocklistSchema`; `features/shared/sdk.ts` (`getSdk`/`buildClientSdkConfig`) + `sdk.server.ts` (`getServerSdk`/`buildServerSdkConfig` + in-memory StorageProvider); DI'd-env pure assemblers. No reads flipped, nothing deleted, `sdk.background` not started. Web suite 129 / SDK suite 625. |
| 12 | S12 | reads-flip (`queryFn → sdk.*`) | ✅ [done](2026-06-13-wallet-sdk-12-reads-flip.md) (preceded by SDK-biome-clean `015a8aaa`; 5 of 6 read-domains flipped, gate green) — new `useSdk()` accessor + flipped `useUser` (canary) / transactions / contacts / exchange-rate / per-quote receive trackers (keys byte-identical; SDK self-resolves session; userId param dropped). **Accounts DEFERRED to S13** (`Account.wallet` is a live wallet-class instance, nominally distinct web↔SDK until S15 — flipping fails `tsc` + would run web money-paths on SDK wallets). `Contact`/`UserProfile` re-pointed to the barrel. **Auth stays on OpenSecret** (canary corrected from `authQueryOptions` → `useUser`: `authState.user` is the OpenSecret identity, not the wallet `User`). Web realtime still drives reactivity. Web suite 129. |
| 13 | S13 | reactivity + orchestration flip (atomic; `useSdkEventBridge` + `background.start/stop` + delete web processor/realtime) | ✅ [done](2026-06-13-wallet-sdk-13-orchestration-flip.md) (9 commits `af0eff89..5a3ab774`, subagent-driven; per-task + opus whole-branch reviews clean) — 5 additive SDK prep (barrel account helpers · `AddAccountConfig` purpose/expiresAt · `send.previewLightningQuote` · `account:updated op:'balance'` · `realtime:connected` + `background.setConnectivity`) + accounts read-flip & `Account`/`ExtendedCashuWallet` unification on the SDK class + `useSdkEventBridge` (§5 table) + the **ATOMIC flip** (mount bridge + `useRealtimeConnectivity`, `sdk.background.start/stop` on the auth lifecycle, flip ALL write mutations + the cashu send-swap read, delete web processor/leader/`use-track-wallet-changes`/`lib/supabase` realtime/all `*ChangeHandlers`/dead list caches/`agicashRealtimeClient`/`useTrackAndUpdateSparkAccountBalances`) + 8b (restore cashu send `destinationDetails` + cashu receive `description`). **Dual-leader invariant verified** (zero `take_lead` in web; `background.start` only in `wallet.tsx`). **Auth STAYS on OpenSecret** (D13-1) → auth-read flip is its own next slice; `entry.client` `configure()` KEPT (plan Step 12 reassigned to the auth slice). Web 129 / SDK 638. **Behaviour proof = e2e + manual money-path: DEFERRED/UNRUN (needs `VITE_BREEZ_API_KEY` + live stack).** |
| 13b | auth-slice | guards off OpenSecret identity onto `sdk.auth`/`sdk.user` + bridge auth handlers + SDK prereqs (cheap `isLoggedIn`/`getCurrentUserId` + `auth:session-expired` producer/scheduler); delete `useHandleSessionExpiry` + `ensureUserData` + web `entry.client` `configure()` | ✅ [done](2026-06-13-wallet-sdk-13b-auth-slice.md) (10 commits 270a7269..b24dbf12, subagent-driven; per-task + opus whole-branch reviews clean; web 129/SDK 651 green, fix:all 0) |
| 14 | S14 | server routes (LN-address → `getServerSdk()`; delete `lightning-address-service.ts` + `database.server.ts`) | ✅ [done](2026-06-13-wallet-sdk-14-server-routes.md) (6 tasks, all reviewed clean — 1 codec [byte-compat fix] + 3 routes + delete-6-files + docs; web 134/SDK untouched, fix:all 0) |
| 15 | S15 | cleanup (delete dead web lib copies, drop unused deps; deferred B1–B6 fixes) | ✅ [done](2026-06-22-wallet-sdk-15-cleanup.md) (12 of 13 tasks executed `d64d6d30..5265db7d`, subagent-driven; all per-task reviews + opus whole-slice review = READY TO MERGE, 0 Crit/0 Imp; net +369/−3661; gate web 136 / SDK 654 / typecheck 4/4 / fix:all 0). **B4 (Task 6, collapse authState.user) DEFERRED to a follow-up per user** (optimization, MEDIUM-risk auth, poor hermetic testability — no jsdom). re-grep corrected the orphan set: most spec-§7 substrate is ALIVE (token-claim route + tx-detail + feature-flags never flipped to sdk.*), so S15 deleted only the genuinely-orphaned ~13 files + ~12 dead exports + 1 dep. biome already done early (`015a8aaa`) |
...
## Plan 10 → S11 / S14 (ServerSdk facade done — Phase 1 complete)

- **(S14 routes own the wire format)** The RR routes (`[.]well-known.lnurlp.$username.ts`, `api.lnurlp.callback.$userId.ts`, `api.lnurlp.verify.$encryptedQuoteData.ts`) call `createServer(config)` (one server instance) and own ALL wire concerns: the LUD JSON envelopes (`payRequest` / `{pr,verify,routes}` / `{status:'OK',settled,preimage,pr}` / `{status:'ERROR',reason}`), the `verify`-URL base prefix (`${origin}/api/lnurlp/verify/${token}`), msat parsing of the callback `amount`, the range-error strings, and the **xchacha20poly1305 encode/decode** of the opaque `$encryptedQuoteData` token (with `LNURL_SERVER_ENCRYPTION_KEY`) over the structured `LnurlVerifyRef` that `ServerSdk` returns/accepts. S14 deletes `lightning-address-service.ts` + the web `cashu/spark-receive-quote-{service,repository}.server.ts` in the same step.
- **(S14 metadata invariant — money-correctness)** The LUD-16 route MUST build its `metadata` from `resolveLightningAddress(...).metadata` (derived from `config.lud16Domain`), because the spark invoice's `descriptionHash` (computed by `ServerSdk` from the same `${username}@${lud16Domain}` string) commits to it. If the route builds metadata from a different host string, the wallet-client's `description_hash` check fails.
- **(S11/S14 config)** The server entry assembles an `SdkConfig` with `supabase.serviceRoleKey`, `serverSparkMnemonic` (from `LNURL_SERVER_SPARK_MNEMONIC`), `breezApiKey`, `sparkStorageDir` (`/tmp/.spark-data`), `lud16Domain`; `openSecret`/`storage`/`defaultAccounts`/`clientId` are unused server-side but still required by the `SdkConfig` type (supply any value, or introduce a narrower `ServerConfig` later if the friction matters).
- **(server spark network)** Verify uses `'MAINNET'`; server spark accounts are mainnet in prod. A REGTEST server receive would need the network carried in the verify ref.
- **(final-review deferral, non-blocking — Important-labeled, controller-adjudicated to Minor)** `ServerAccountRepository.getDefaultAccount` joins `cashu_proofs(*)` it never reads (mirrors the production web repo). Optional bandwidth/clarity cleanup: drop the proofs sub-select if it proves costly on the LN-address callback path (requires loosening `isCashuAccount`/`isSparkAccount` to accept a proof-less row, or a scoped cast). No correctness impact (no `!inner` → the embedded filter can't drop accounts; proofs are never decrypted).
- **(carried Minors, non-blocking)** server repos: `rejects.toBeDefined()` weak error assertion (T2); cashu repo test omits a `p_transfer_id`-absent assertion (T3); no CASHU_TOKEN-branch test on the server services (T4/T5 — server flow is LIGHTNING-only); `server-connections.ts` `CashuWalletService` could be `import type` (T7); `getCashuMintWallet` redundant arrow wrapper (T8). All cosmetic/coverage.

## Plan 11 → S12 / S13 / S14 / S15 (web entry points done — Phase 2 begun)

- **(S12 reads)** Flip each web read `queryFn` to `sdk.*`, threading `getSdk(domain)` where `domain = useRouteLoaderData('root').domain` (the Vercel-aware canonical-origin host). Safest first canary = `authQueryOptions` → `sdk.user.getCurrentUser()` (null ⇒ `{isLoggedIn:false}`; user ⇒ `{isLoggedIn:true,user}`); the logged-in branch is only provable via e2e/manual (live OpenSecret session). `useUser`/`useAccounts` need an already-resolved `user.id` so they flip after auth-state. Web realtime still drives reactivity through S12.
- **(S13 atomic flip — real-money guardrail)** Mount the one `useSdkEventBridge(queryClient)`, flip write mutations, AND in the SAME step `sdk.background.start()` on sign-in / `stop()` on sign-out while DELETING the web TaskProcessor + `useTakeTaskProcessingLead` + `use-track-wallet-changes` + all `*ChangeHandlers` + the web realtime wiring. Never run the web processor and `sdk.background` together (dual leaders / double melt-mint — spec §8). `getSdk()` caches a `Promise<Sdk>`; `background.start/stop` are auth-lifecycle (D10). Also remove the now-redundant `entry.client.tsx` `configure()` (the SDK owns it via `buildConnections`).
- **(S14 server routes)** Wire the three LN-address routes to `getServerSdk(domain)` (domain from the request origin host, matching `root.tsx`'s `getCanonicalOrigin` precedence so the spark `descriptionHash`/LUD-16 `metadata` invariant holds — S10 carryover). Routes keep the LUD JSON wire format + `LNURL_SERVER_ENCRYPTION_KEY` xchacha verify-token over the SDK's structured `LnurlVerifyRef`. Delete `lightning-address-service.ts` + `database.server.ts`. The `getServerSdk` singleton freezes `lud16Domain` on first request (stable per prod origin; flagged Minor — build per-request if per-request domains ever matter). Latent S10 currency guard: a `bypassAmountValidation=true` USD-spark default account is unreachable today but unguarded in `ServerSdk`.
- **(S15 cleanup)** Delete the web's `defaultAccounts` const (`user-hooks.tsx`), the web's `~/lib/cashu/mint-validation` copy (the S11 assembler now imports `MintBlocklistSchema` from `@agicash/wallet-sdk`), other dead web lib copies; drop unused deps.
- **(GATE — blocks every Phase-2 slice; needs a decision)** `bun run fix:all` is biome-ONLY (no typecheck) AND the SDK package was never biome-checked in Phase 1 (Plans 01–10 gated on `typecheck`+`test`). So monorepo-root `fix:all` reformats ~53 `packages/wallet-sdk/src/**` files + exits non-zero on unfixable SDK lint — independent of any web change. S11 used a **web-scoped biome gate** + explicit `typecheck` + web suite + SDK suite. **A dedicated SDK-biome-cleanup task (biome `check --write` over `packages/wallet-sdk` + fixing the unfixable lint) should land before/within S12** so the monorepo-wide `fix:all` gate is honest again.

## Plan 12 → S13 / S14 / S15 (reads-flip done — 5 of 6 read-domains on the SDK)

- **(GATE resolved)** The Phase-1 SDK-biome debt (Plan 11 carryover) was cleared FIRST as `chore(wallet-sdk): biome-clean the package` (`015a8aaa`): the SDK package now passes the same strict biome config as the rest of the monorepo (75 empty-stub `=> undefined` + non-null guards + 1 intentional-thenable ignore; SDK 625 green). Monorepo-root `bun run fix:all` is now honest (exit 0). So the Phase-2 gate is the full `bun run fix:all` + `bun run typecheck` + web suite (+ SDK suite if SDK touched). Row 48's "biome the SDK" is therefore already done early — S15 need not repeat it.
- **(S12 done)** A shared `useSdk(): Promise<Sdk>` accessor (`features/shared/use-sdk.ts`; domain via `useLocationData()`) backs every flipped read. Flipped (keys byte-identical, queryFn only): `useUser`→`getCurrentUser`, `useTransaction(s)`/`countPendingAck`, `useContacts`/`search`, the exchange-rate shared queryFn, both receive per-quote trackers. SDK reads self-resolve the session (no userId param); existing `useUser()`/`_protected` gating preserved. `Contact`/`UserProfile` minimally re-pointed to the barrel (+ web `toContact` emits `Date`). Auth + feature-flags untouched.
- **(S13 — accounts read flips HERE, not S12)** `Account.wallet` is a live wallet-class instance; the SDK's `ExtendedCashuWallet`/`BreezSdk` are nominally distinct from the web's own copies (private `_bip39Seed`) which coexist until S15. Flipping `accounts.list()` in S12 fails `tsc` and cascades into `getExtendedAccounts`/`getAccountBalance`/`AccountService`, the wallet classes, AND `claim-cashu-token-service.ts` — and would run the web money-paths on SDK-built wallets. So `useAccounts`/`useBalance`/`useAccountOrNull` flip at **S13**, in the SAME step the web wallet/connection classes are deleted and `Account` unifies on the SDK type. The original step-by-step is retained in the Plan 12 doc's Task 2 (marked DEFERRED) as S13 reference. Note: `claim-cashu-token-service.ts` is a 2nd consumer of `accountsQueryOptions`.
- **(S13 atomic flip)** Mount the one `useSdkEventBridge(queryClient)`, flip write mutations, AND `sdk.background.start()`/`stop()` on sign-in/out while DELETING the web TaskProcessor + `useTakeTaskProcessingLead` + `use-track-wallet-changes` + all `*ChangeHandlers` + the `{Entity}Cache` classes + the web realtime wiring + `entry.client.tsx` `configure()`. Never run the web processor and `sdk.background` together (dual leaders / double melt-mint — spec §8). The bridge must drive the SAME keys S12 preserved (`['user']`, `['all-transactions',accountId]`, `['unacknowledged-transactions-count']`, `['transactions',id]`, `['contacts']`, `['cashu-receive-quote',id]`, `['spark-receive-quote',id]`) plus `['accounts']` once accounts flips. The contacts cache is naive-append/no-version — the forwarder's `toContact`-equivalent must emit `Date` (S12 made web `Contact.createdAt` a `Date`).
- **(S13 auth-read flip — now a real work item)** Flipping `authQueryOptions` was deferred (OpenSecret identity ≠ wallet `User`). A dedicated step must migrate the route guards (`_auth.tsx` login_method/email, `_protected.tsx` shouldUserVerifyEmail/hasUserChanged/bootstrap) off the OpenSecret identity fields and re-home the `Sentry.setUser`/session-hint-cookie side-effects before routing auth through `sdk.user.getCurrentUser()`.
- **(S13 fixup)** `claim-cashu-token-service.ts` now calls `~/lib/exchange-rate` `exchangeRateService.getRates` directly (bypasses the TanStack cache) because it has no domain/sdk in S12 — thread `sdk` into the service ctor at S13 (domain accessible in the route factory that instantiates it).
- **(S14 server routes)** Unchanged from the Plan 11 carryover (wire the 3 LN-address routes to `getServerSdk(domain)`; keep the LUD wire format + xchacha verify-token; delete `lightning-address-service.ts` + `database.server.ts`).
- **(S15 cleanup)** Delete the web's now-dead reads/repos/lib copies (incl. `~/lib/exchange-rate` once its remaining direct importers migrate, the dead `findContactCandidates`/`getExtendedAccounts` paths, the web `defaultAccounts` const, the `~/lib/cashu/mint-validation` copy), drop unused deps, final `fix:all`. Minor S12 polish to fold in: restore the dropped `exchangeRatesQueryOptions` normalization JSDoc; tidy the `contact.ts` `isContact` inline-`import()` return type.

## Plan 13 → auth-slice / S14 / S15 (orchestration flip done — web fully SDK-driven on the client)

- **(S13 done)** The web client is now SDK-driven: one `useSdkEventBridge(queryClient)` maps `sdk.events` → the TanStack cache (§5 table, against the byte-identical S12 keys + `['accounts']`), all write mutations call `sdk.*`, `sdk.background.start()/stop()` runs on the authed-user lifecycle in `Wallet`, and the entire web orchestration/realtime/change-handler/processor/leader stack is deleted. `Account`/`ExtendedCashuWallet` unified on the SDK class (web `lib/cashu/utils.ts` re-exports the barrel `ExtendedCashuWallet` value+type). A thin `useRealtimeConnectivity` drives `sdk.background.setConnectivity` (window online/offline + visibility); reconnect catch-up via the `realtime:connected` event; spark balance-only refresh via `account:updated op:'balance'`. The **dual-leader money-safety invariant** (spec §8) is verified: the web no longer polls `take_lead`; `sdk.background` is the sole leader, started atomically with the processor deletion.
- **(auth slice — PLANNED, slice `13b`)** Plan written: [`2026-06-13-wallet-sdk-13b-auth-slice.md`](2026-06-13-wallet-sdk-13b-auth-slice.md) (grounding + 6 resolved forks in the `project-wallet-sdk-auth-slice-grounding` memory). Auth deliberately stayed on OpenSecret through S13 (D13-1). The slice migrates the route guards (`_auth.tsx`, `_protected.tsx`) off the OpenSecret identity onto `sdk.auth.isLoggedIn()` (new cheap surface) + `sdk.user.getCurrentUser()` (bootstraps internally — `resolveSession` IS `ensureUserData`); flips `authQueryOptions.queryFn`→`getCurrentUser` and makes `useAuthActions` a thin adapter over `sdk.auth.*` (F3: KEEP both per §7b, not full-retire); flips the OAuth callback→`sdk.auth.completeOAuth`; threads terms through the signup ACTIONS (F4). **SDK prereqs (Tasks A1–A3):** cheap network-free `sdk.auth.isLoggedIn()`/`getCurrentUserId()` + a DEDICATED per-instance `SessionExpiryScheduler` producing `auth:session-expired` (F1; guests self-heal silently, F2). `login_method` PROVEN debug-only → dropped (no SDK surface). Un-stub the bridge auth handlers (`auth:signed-in`→invalidate `['auth-state']`+`['feature-flags']`; `auth:signed-out`→`clear()`+Sentry; `auth:session-expired`→toast+`signOut`) and DELETE `useHandleSessionExpiry` + `ensureUserData` + web `entry.client.tsx` `configure()` (§8.5 Step 12, reassigned here) — `configure()`-delete proven SAFE (idempotent setter). **The `CompletedSparkLightningSendTransactionDetails` type-bug fix is DEFERRED to S15 (F5).** Behaviour gate (F6: `test:e2e` + manual money-path, covering S13's unrun paths too) runs BEFORE push/PR.
- **(S14 server routes)** Unchanged (wire the 3 LN-address routes to `getServerSdk(domain)`; keep the LUD wire format + xchacha verify-token; delete `lightning-address-service.ts` + `database.server.ts`).
- **(S15 cleanup — S13 additions)** Now-orphaned-but-build-valid web code to delete: `useCreateCashuReceiveSwap`, `useCreateCrossAccountReceiveQuotes` (collapsed into `receiveToken`), `transfer-service.ts`, `transaction-repository.ts`, and the `shared/cashu.ts`/`shared/spark.ts` wallet-construction helpers + the domain repos once the auth-bootstrap (`_protected.ensureUserData`) and S14 server importers move off them. Stale `account-hooks.ts` `structuralSharing` comment ("getAllActive" → `sdk.accounts.list()`). The barrel account value-helpers (Task 1) are exported but the web still keeps byte-equivalent locals — consume the barrel or drop the exports.
- **(SDK follow-ups discovered in S13)** (a) **Public-type bug:** `CompletedSparkLightningSendTransactionDetails = Required<…> & {transferId?}` ineffectively over-requires `transferId` (the intersection collapses to required); the internal db schema is correct. Worked around behavior-free via a contained 2-consumer `Transaction` re-point for the acknowledge flip. Fix SDK-side: `Omit<Required<…>,'transferId'> & {paymentPreimage: string; transferId?: string}`, then the web can unify `Transaction` on the barrel globally. (b) `send.previewLightningQuote`/`createLightningQuote` `destinationDetails` passthrough + cashu `receive.createLightningQuote` `description` — ADDED in S13 (Task 8b); spark send never persisted `destinationDetails` (no schema field) so it was correctly left alone.
- **(deferred optimizations)** `BackgroundRunner.registerBalanceListeners` snapshots accounts once at `start()` — a late-online spark account misses balance updates until `stop()+start()`; re-register on `account:created/updated` if it bites. A per-tab-stable `SdkConfig.clientId` (sessionStorage) would close the up-to-6s zombie-lock window on remount/HMR/relogin (SDK already honors a passed `clientId`).
- **(GATE — S13 is NOT yet behaviour-proven)** Unit/typecheck/biome pass (web 129 / SDK 638), but the web suite has no jsdom and S13 changed the money paths. The REAL gate (spec §10) is `bun run test:e2e` + manual money-path via Chrome DevTools MCP — **DEFERRED/UNRUN** (needs `VITE_BREEZ_API_KEY` + a live stack). Exercise specifically: send-to-contact + send-to-ln-address (history label restored by 8b), cashu BUY_CASHAPP (memo restored by 8b), token-receive across all 3 union branches, spark balance refresh after a receive, reconnect catch-up after offline→online, and logout→login-as-another-user (background restart). Do NOT push the branch / open the PR until this gate is run.

## Plan 14 → S15 (server routes done)

- **(a) S15 dead-code orphans to prune:** `ReadUserDefaultAccountRepository` in `apps/web-wallet/app/features/user/user-repository.ts` (zero consumers after S14 deleted the web server receive repos that called it); `sparkWalletQueryOptions` export (external consumers drop to zero after S14 — could be made module-private).
- **(b) Optional boot-time `VITE_SUPABASE_URL` guard:** deleting `database.server.ts` shifted the validation of `VITE_SUPABASE_URL` from startup to lazy/first-request inside `buildServerSdkConfig`/`getServerSdk`. Accepted as-is; add an explicit startup guard in `getServerSdk` if eager fail-fast is later desired.
- **(c) T1-M1: add 64-hex-char (32-byte) validation on `LNURL_SERVER_ENCRYPTION_KEY`** in the lazy accessor in `lnurl-verify-token.server.ts` — a malformed key currently fails inside the XChaCha20-Poly1305 cipher on first use rather than at startup.
- **(d) `/lnurl-test` production gate (F2) DEFERRED:** the canonical-origin (`agi.cash`) branch of `getCanonicalOrigin` cannot be exercised from localhost. This gate is folded into the combined S13 + auth-slice F6 money-path gate (needs `VITE_BREEZ_API_KEY` + live stack + user nod). Do NOT push/PR until it runs.

## Plan 15 → done (cleanup slice complete — Phase 2 code-complete)

- **S15 DONE** (`d64d6d30..5265db7d`, 13 commits, subagent-driven; all per-task reviews + opus whole-slice review = READY TO MERGE, 0 Crit/0 Imp; web 136 / SDK 654 / typecheck 4/4 / fix:all 0; net +369/−3661). Landed B1 (`CompletedSparkLightningSendTransactionDetails.transferId`→optional + web `Transaction` barrel-unify), B2 (`bootstrapUser` retry), B3 (internalize `sessionExpiry` seam off public `SdkConfig`), B5 (lnurl codec 64-hex guard), B6 (eager `VITE_SUPABASE_URL/ANON_KEY` guards); deleted the orphaned web code (transfer-service + dead send-quote service classes/repo, 4 cashu subscription managers, the web exchange-rate dir [Ticker re-pointed to the SDK barrel], password-generator/with-retry/use-long-timeout/timeout/contact-repository/cashu-receive-swap-hooks, empty lib/supabase) + stripped ~12 dead named exports + dropped unused `@radix-ui/react-scroll-area`.
- **CARRYOVER CORRECTION (re-grep on today's tree):** most of spec §7's "web deletes" substrate is ALIVE and STAYS — the token-claim route `_protected.receive.cashu_.token.tsx`, the transaction-detail chain (`transaction-additional-details` + `useReverseTransaction`), and `feature-flags.ts` were never flipped to `sdk.*`, so they still use `account/user/receive` repos+services, `claim-cashu-token-service`, `shared/{cashu,spark,encryption,cryptography,auth}`, `agicash-db/*`, `lib/{ecies,bolt11,lnurl,spark,sha256,xchacha20poly1305}`, most of `lib/cashu` incl. `mint-validation.ts`. Deleting these is FEATURE work (migrate those routes onto `sdk.*`), NOT cleanup — left for a future slice. Only `@radix-ui/react-scroll-area` was an unused dep; all crypto/cashu/spark/supabase deps stay used.
- **(B4 DEFERRED — Task 6, user decision 2026-06-22):** collapsing `authState.user` onto the `['user']` query is an optimization (not a correctness fix), MEDIUM-risk auth, and hard to test hermetically (no jsdom; `authQueryOptions.queryFn` is window/Sentry/cookie/localStorage-bound). → standalone follow-up where it can get a proper test or live verification.
- **(Minor follow-ups, non-blocking, from the opus whole-slice review):** (1) `transaction-repository.ts` is now a husk holding only `export type Cursor` — optional: relocate `Cursor` into `transaction-hooks.ts` and delete the file; (2) two `Transaction` types coexist in the transactions feature (SDK barrel vs local `transaction.ts`, still used by `transaction-ack-status-store.ts`) — expected residual until a later substrate-flip slice; (3) a couple of duplicated `import type` statements from `@agicash/wallet-sdk` (cosmetic, biome-accepted); (4) pre-existing JSDoc typo `user-service.ts:3` (`user'sdefault`), not introduced by S15.
- **GATE before push/PR (unchanged, still OWED):** the combined S13 + auth-slice + S14 + S15 live money-path test — `bun run test:e2e` + manual Chrome-DevTools money paths + `/lnurl-test` against PROD `https://agi.cash` (needs `VITE_BREEZ_API_KEY` + live stack). The whole no-cache migration is ONE PR; it is now CODE-COMPLETE but **must not be pushed/PR'd until that gate runs + user nod.**
