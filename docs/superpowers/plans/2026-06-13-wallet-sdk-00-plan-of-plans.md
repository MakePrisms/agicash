...
- **(b)** S9's forwarder must **NOT double-drive** `contact:created`/`contact:deleted` — S8 emits them **synchronously** from `add`/`remove` (Decision D5). The forwarder must gate on the event source (SDK-initiated vs. server-written) or skip contacts entirely.
- **(c)** `transfers.executeQuote` **re-derives fresh quotes** from the slim `TransferQuote` (Decision D3 — re-quote drift vs. the preview is accepted, since the slim quote carries no live lightning quotes and the SDK is stateless per call). If fee-stable execution is later required, a contract change would be needed to carry the live quote objects.
