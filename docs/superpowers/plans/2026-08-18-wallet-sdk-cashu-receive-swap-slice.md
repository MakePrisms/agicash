...
1. **Slice scope = the cashu-receive-swap domain, whose only host-initiated verb is swap creation.** Evidence: repo-wide grep shows exactly three web-side entry points into `CashuReceiveSwapService.create`: (a) `useCreateCashuReceiveSwap` (`cashu-receive-swap-hooks.ts:99`), called from the **same-account claim branch** of `receive-cashu-token.tsx:166–173` (`isClaimingToSameCashuAccount` → swap on the mint's `/v1/swap`, NUT-03); (b) in-package `claim-cashu-token-service.ts:125` (the token route's browser `clientLoader` claim path via `_protected.receive.cashu_.token.tsx` — step 12, per the plan-attack correction; the Lightning-address server claim is the separate `LightningAddressService`, step 17); (c) in-package `cashu-send-swap-service.ts:288` (send reversal, step 14). Step 10 flips (a) only. Neighbors on the spec's step list stay put: spark receive quote = step 11 (`spark-receive-quote-hooks.ts`), cross-account token claim (`createCrossAccountReceiveQuotes` in `receive-cashu-token-hooks.ts` + the token route) = step 12, cashu/spark sends = steps 13–15, transfer = step 16.
...
3. **Contract params take the full account and the parsed token:** `CreateCashuReceiveSwapParams = { account: CashuAccount, token: Token }` (param precedent, Global constraints). The caller-object rule extends through the web wrapper (plan-attack finding 1): the hook's `CreateProps` becomes `{ token: Token; account: CashuAccount }` and `receive-cashu-token.tsx` passes the account object it already holds — no id round-trip through `useGetCashuAccount`, no second source of truth, matching the merged step-9 web pattern (`cashu-receive-quote-hooks.ts` passes `account` straight through). `reversedTransactionId` is deliberately **not** on the params: its only supplier is in-package `cashu-send-swap-service.ts:288` (never the host), the same reasoning that pinned `receiveType: 'LIGHTNING'` inside the step-9 API. Step 14 keeps reaching the service in-package.
4. **Return type is the service's `{ swap: CashuReceiveSwap; account: CashuAccount }`, exported as `CreateCashuReceiveSwapResult`.** The domain entities ride along per the standing note in `domain/sdk/receive.ts:9–11` (#1164 narrows later). `receive-cashu-token.tsx:167–172` destructures `swap.transactionId` from exactly this shape, so the caller's result handling keeps working unchanged.
...
8. **Web flip = the hook plus its sole caller.** `useCreateCashuReceiveSwap` (`cashu-receive-swap-hooks.ts:99–118`): the mutation body calls `sdk.receive.cashu.createSwap({ token, account })`; the `useUser`, `useCashuReceiveSwapService`, and any account-lookup locals in that hook are deleted; `CreateProps` becomes `{ token: Token; account: CashuAccount }`; the return shape is unchanged. `receive-cashu-token.tsx` (the only caller): the same-account claim branch passes the `CashuAccount` it already holds, narrowed at the call site with an `account.type === 'cashu'` check (no cast — `isClaimingToSameCashuAccount` returns a plain boolean, not a type guard); the newly-added-account branch passes the account object returned by `addAndSetReceiveAccount` directly instead of an id. Error behavior is unchanged: the same repository throws the same `UniqueConstraintError('This token has already been claimed')`, surfaced by the caller's existing toast.
...
10. **Untouched laggard consumers** (each has its own slice): `transaction-additional-details.tsx` (`getByTransactionId` is not on the contract), `send/cashu-send-swap-hooks.ts` (builds `CashuReceiveSwapService` for reversal wiring, step 14), `receive-cashu-token-hooks.ts` + the `routes/_protected.receive.cashu_.token.tsx` `clientLoader` + `claim-cashu-token-service.ts` (cross-account claim, route loader, and its same-account swap branch — all step 12), `spark-receive-quote-hooks.ts` (step 11), `lightning-address-service.ts` + the LNURL server routes (step 17). Note: `receive-cashu-token.tsx` itself is IN scope for the param flip (decision 8) — only its claim orchestration beyond the mutation call stays untouched.
...
Web flip (`apps/web-wallet/app/features/receive/cashu-receive-swap-hooks.ts`): add `import { sdk } from '~/features/shared/sdk.client';`, change `CreateProps` to `{ token: Token; account: CashuAccount }` (import types as needed), and replace the body of `useCreateCashuReceiveSwap`:
...
Caller flip (`apps/web-wallet/app/features/receive/receive-cashu-token.tsx`): update the two `createSwap`/mutation call sites to pass the account object instead of `accountId` — the same-account branch narrows the account it already holds with an `account.type === 'cashu'` check (no cast), and the newly-added-account branch passes the object returned by `addAndSetReceiveAccount` directly. Nothing else in the component changes (result handling already destructures `{ swap }`).
...
- Modify: `apps/web-wallet/app/features/receive/receive-cashu-token.tsx` (caller passes the account object, decision 8)
- Untouched on purpose: `packages/wallet-sdk/index.ts`, `domain/sdk/sdk.ts`, `domain/sdk/index.ts`, `domain/sdk/events.ts`, `sdk.test.ts`, `cashu-receive-swap-repository.ts`, `cashu-receive-swap.ts`, `claim-cashu-token-service.ts`, `cashu-send-swap-service.ts`, all other web `.tsx` components, `receive-cashu-token-hooks.ts`, `send/cashu-send-swap-hooks.ts`, `transaction-additional-details.tsx`, `task-processing.ts`, `use-track-wallet-changes.ts`, `sdk.client.ts`, all RPCs and migrations.
...
**Task 1 (contribution, single seller): the whole slice.** Edit exactly the seven files in the file map, applying the pinned seams verbatim. Working order and requirements:
...
3. **Tests.** Extend `receive-api.test.ts` in its existing harness style (fake `getSession`, real `createSessionKeys` with controllable session scope, seam injection). Add a `cashu.createSwap` describe plus fence cases in the existing `session fences` describe. Required coverage:
   ...
   - (h) abort-signal forwarding through the default service (plan-attack finding 3): exercise the **default** `createSwapService` path with the `createSwapRepository` seam injecting a fake repository whose `create` records its second argument; assert the repository receives the same object `keys.sessionSignal()` returned — this locks the new service-to-repository `options` hop, which case (b)'s fake service cannot see.
4. **Web flip.** Apply the pinned hook body and the caller flip in `receive-cashu-token.tsx`. The flipped hook must not reference `useCashuReceiveSwapService`, `useUser`, or any account lookup; every other export in the file stays byte-identical (the processor and change handlers are the step-18 boundary).

Gates in the seller's fork, all mandatory: `bun install`; `bun run fix:all` exit 0; `bun run typecheck` exit 0; `cd packages/wallet-sdk && bun test` green. The changed-file set of the delivered branch must equal the seven-file list exactly (the orchestrator verifies with `git archive` + `diff -rq`).
