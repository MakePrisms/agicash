...
## Recipe corrections (learned from the money extraction — apply to every package)

Three gaps surfaced while extracting `@agicash/money`; fold these into each remaining package's plan:
...
3. **The verification gate is `bun run typecheck` — NOT `bun run fix:all`.** `fix:all` is `biome check --write` (lint + format ONLY) — it does not type-check and will not catch a dangling import. The real gate is `bun run typecheck` (`react-router typegen && tsc` for the app, `tsc` for packages), and `tsc` *does* catch missing modules in EVERY form — alias, relative, value, and type-only. (An earlier diagnosis that "tsc-Bundler doesn't check relative paths" was wrong; the truth is we'd only ever run `fix:all`, which never type-checks.) The production build is NOT a substitute: Vite/esbuild **erases `import type` before resolution**, so a build silently passes over type-only dangling imports (it only flags missing *value* imports). So every extraction must run **`bun run typecheck` + `bun run test`** (a `bun --filter='web-wallet' run build` is a useful extra check for value-import resolution but may fail on this worktree's missing `VITE_*` env — non-blocking). The money extraction initially shipped 3 dangling `import type … from '../money'` precisely because its gate was `fix:all`; fixed in `509753c1`.

**Deferred follow-up (not part of lib extraction):** relocate `registerDevToolsFormatter` (the `window.devtoolsFormatters` block in `packages/money/src/money.ts`) out of the shared lib into the app's browser entry, so `@agicash/money` can drop `"DOM"` from its `tsconfig` lib and be truly headless. Left as-is here because this task was move-not-refactor.
