 ...
 */
export function deriveNut20LockingPublicKey(xPub: string): {
  lockingPublicKey: string;
  fullLockingDerivationPath: string;
} {
  // Generate a cryptographically random unhardened index in [0, HARDENED_OFFSET).
  // Use 4 random bytes and mask to stay below HARDENED_OFFSET (0x80000000).
  const buf = randomBytes(4);
  const view = new DataView(buf.buffer);
  const unhardenedIndex = (view.getUint32(0) & 0x7fffffff) % HARDENED_OFFSET;

  const hdKey = HDKey.fromExtendedKey(xPub);
  const childKey = hdKey.derive(`m/${unhardenedIndex}`);
