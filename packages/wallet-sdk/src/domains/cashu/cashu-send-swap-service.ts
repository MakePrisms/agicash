...
import { getTokenHash } from '../../internal/lib/cashu/token';
...
export class CashuSendSwapService {
  ...
  /**
   * Estimates the cashu swap fee required to send the given amount.
   * @throws {DomainError} with code 'insufficient_balance' if the account does not have enough balance.
   */
  async getQuote({
    account,
    amount,
  ...
  /**
   * Swaps the input proofs for exact proofs to send, moving the swap from DRAFT → PENDING.
   * @throws if the swap is not DRAFT or does not belong to the account.
   */
  ...
  /**
   * Completes the swap (PENDING → COMPLETED).
   * No-op if already COMPLETED.
   * @throws if the swap is not PENDING.
   */
  async complete(swap: CashuSendSwap) {
    ...
    if (swap.state !== 'PENDING') {
      throw new DomainError(
        `Swap is not PENDING. Current state: ${swap.state}`,
        'invalid_state',
      );
    }
  ...
  /**
   * Fails the swap (DRAFT → FAILED).
   * No-op if already FAILED.
   * @throws {DomainError} with code 'invalid_state' if the swap is not DRAFT.
   */
  async fail(swap: CashuSendSwap, reason: string) {
    ...
    if (swap.state !== 'DRAFT') {
      throw new DomainError(
        `Swap is not DRAFT. Current state: ${swap.state}`,
        'invalid_state',
      );
    }
  ...
  /**
   * Reverses a PENDING swap by creating a receive swap for the proofsToSend,
   * effectively reclaiming the tokens back into the account.
   * No-op if already REVERSED.
   * @throws {DomainError} with code 'invalid_state' if the swap is not PENDING.
   * @throws {DomainError} with code 'account_mismatch' if the swap does not belong to the account.
   */
  async reverse(swap: CashuSendSwap, account: CashuAccount): Promise<void> {
    ...
    if (swap.accountId !== account.id) {
      throw new DomainError(
        'Swap does not belong to account',
        'account_mismatch',
      );
    }
