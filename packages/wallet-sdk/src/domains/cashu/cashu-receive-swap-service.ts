...
import { tokenToMoney } from '../../internal/lib/cashu/token';
import type {
  CashuReceiveSwap,
...
export class CashuReceiveSwapService {
   ...
   * @throws {DomainError} with code 'mint_mismatch' if the token mint doesn't match the account mint.
   * @throws {DomainError} with code 'currency_mismatch' if the token currency doesn't match the account currency.
   * @throws {DomainError} with code 'token_too_small' if the token amount is too small to cover fees.
   */
  async create({
    userId,
    token,
    ...
    if (amountToReceive <= 0) {
      throw new DomainError('Token is too small to claim.', 'token_too_small');
    }
   ...
   * @throws {DomainError} with code 'invalid_state' if the swap is not in a PENDING state.
   */
  async fail(
    swap: CashuReceiveSwap,
    reason: string,
   ...
   * @throws {DomainError} with code 'invalid_state' if the receive swap is not pending.
   */
  async completeSwap(
    account: CashuAccount,
    ...
    if (receiveSwap.state !== 'PENDING') {
      throw new DomainError('Receive swap is not pending', 'invalid_state');
    }
