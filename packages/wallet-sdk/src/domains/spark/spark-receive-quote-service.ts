...
export class SparkReceiveQuoteService {
  ...
  /**
   * Creates a Spark receive quote (persisted in UNPAID state) for the resolved
   * lightning quote.
   * @param params - The receive-quote params, discriminated by `receiveType`
   *   (`LIGHTNING` or `CASHU_TOKEN`).
   * @returns The created receive quote.
   */
   ...
   * @throws {DomainError} code `invalid_state` if the quote is not in UNPAID state.
   */
  async complete(
   ...
   * @throws {DomainError} code `invalid_state` if the quote is not in UNPAID state or has not expired yet.
   */
   ...
   * @returns The updated quote.
   * @throws {DomainError} code `invalid_state` if the quote is not of type CASHU_TOKEN or is not in UNPAID state.
   */
