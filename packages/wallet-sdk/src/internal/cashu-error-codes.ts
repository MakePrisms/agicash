/**
 * Cashu NUT error codes — INTERNAL to the SDK (§12: `lib/cashu` is internal-only).
 *
 * Lifted VERBATIM from `apps/web-wallet/app/lib/cashu/error-codes.ts` (the canonical
 * source maintained against https://github.com/cashubtc/nuts/blob/main/error_codes.md).
 * Consumed by `src/classify.ts` to map mint-emitted error codes into the 4-bucket
 * `classify()` verdict. NOT part of the public barrel.
 *
 * TODO(follow-up): when `app/lib/cashu/**` is relocated into the package wholesale,
 * fold this back into the single internal `lib/cashu` so there is one copy.
 */
