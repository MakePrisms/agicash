...
import type { TaskProcessorApi } from './task-processor';
...
export type SdkConfig = {
  ...
  auth: {
     ...
     * SDK's CSPRNG generator. Test seam for host-supplied guest credentials
     * (e.g. an e2e password mock).
     */
...
export type Sdk = {
  ...
  readonly taskProcessor: TaskProcessorApi;
  /**
  ...
  init(): Promise<void>;
  /**
   * Awaits in-flight task-processor transitions to their next checkpoint, then
   * tears down realtime + the task processor; still-pending namespace promises
   * reject with a typed `SdkError`.
   */
