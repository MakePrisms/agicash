...
export class ReceiveCashuTokenService {
  constructor(
    ...
    private readonly getFeatureFlag: (flag: string) => boolean,
    private readonly mintValidator: MintValidator,
  ) {}
  ...
  async buildAccountForMint(
    ...
    const validationResult = this.mintValidator(
      mintUrl,
      unit,
      mintInfo,
    ...
    const isTestMint = await this.cache.fetchQuery({
    ...
    const isGatedGiftCard =
      wallet.purpose === 'gift-card' && !this.getFeatureFlag('GIFT_CARDS');
  ...
  async getSourceAndDestinationAccounts(
    ...
    if (existingCashuAccount) {
      const isGatedGiftCard =
        ...
        !this.getFeatureFlag('GIFT_CARDS');
      const sourceAccount = {
