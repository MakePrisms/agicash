...
/**
 * Minimal interface for the user service dependency.
 */
type UserServiceLike = {
  setDefaultAccount(
    user: User,
    account: Account,
    options: { setDefaultCurrency?: boolean; abortSignal?: AbortSignal },
  ): Promise<User>;
};
...
export class ClaimCashuTokenService {
  constructor(
    ...
    private readonly userService: UserServiceLike,
    private readonly getExchangeRate: (pair: string) => Promise<string>,
  ) {}
  ...
  private async handleClaim(
    ...
    const accounts = await this.cache.fetchQuery({
      queryKey: ['accounts'],
      queryFn: () => this.accountRepository.getAll(user.id),
    ...
    if (
      ...
      if (result.success) {
        this.cache.setQueryData?.(['user'], result.user);
      }
    }
    ...
    if (isSameAccountClaim) {
      ...
      // Account cache update is handled by the web layer hook wrapper
  ...
  private async tryCompleteReceive(
    ...
    try {
      ...
      if (quotes.destinationType === 'spark') {
        ...
        // To make sure the new balance is immediately reflected in the UI, we invalidate the spark balance query.
        await this.cache.invalidateQueries?.({
          queryKey: ['spark-balance', quotes.destinationAccount.id],
        });
        return { success: true };
      }
    } catch (error) {
