...
import type { Keyset, MintKeyset, Token } from '@cashu/cashu-ts';
import { ExtendedMintInfo, type MintPurpose } from './protocol-extensions';
import { CASHU_PROTOCOL_UNITS, type CashuProtocolUnit } from './types';

export const currencyToUnit: {
...
export const getWalletCurrency = (unit: string): Currency => {
  const key = unit as keyof typeof cashuProtocolUnitToCurrency;
  if (!cashuProtocolUnitToCurrency[key]) {
    throw new Error(`Unsupported cashu wallet unit: ${unit}`);
  }
  return cashuProtocolUnitToCurrency[key];
};
