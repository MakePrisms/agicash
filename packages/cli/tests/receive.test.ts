...
function makeMockCtx(overrides: Partial<SdkContext> = {}): SdkContext {
  return {
    ...
    } as unknown as SdkContext['accountRepo'],
    cashuReceiveQuoteRepo: {
      getPending: async () => [],
      get: async () => null,
    } as unknown as SdkContext['cashuReceiveQuoteRepo'],
...
describe('receive validation', () => {
  test('rejects missing input', async () => {
    const r = await handleReceiveCommand(makeArgs(), makeMockCtx());
    expect(r.action).toBe('error');
    expect(r.code).toBe('MISSING_INPUT');
  });
  test('rejects invalid input', async () => {
    const r = await handleReceiveCommand(makeArgs(['abc']), makeMockCtx());
    expect(r.action).toBe('error');
    expect(r.code).toBe('INVALID_AMOUNT');
  });
  test('rejects zero amount', async () => {
    const r = await handleReceiveCommand(makeArgs(['0']), makeMockCtx());
    expect(r.action).toBe('error');
    expect(r.code).toBe('INVALID_AMOUNT');
  });
  test('rejects when no accounts', async () => {
    const r = await handleReceiveCommand(makeArgs(['100']), makeMockCtx());
    expect(r.action).toBe('error');
    expect(r.code).toBe('NO_ACCOUNT');
  });
  test('rejects unknown account', async () => {
    const r = await handleReceiveCommand(
      makeArgs(['100'], { account: 'x' }),
      makeMockCtx(),
    );
    expect(r.action).toBe('error');
    expect(r.code).toBe('NO_ACCOUNT');
  });
  test('detects cashu token', async () => {
    const r = await handleReceiveCommand(
...
describe('receive list', () => {
  test('returns empty when no pending', async () => {
    const r = await handleReceiveCommand(makeArgs(['list']), makeMockCtx());
...
describe('receive --check-all', () => {
  test('returns zero summary', async () => {
    const r = await handleReceiveCommand(
      makeArgs([], { 'check-all': true }),
      makeMockCtx(),
    );
    expect(r.action).toBe('checked');
