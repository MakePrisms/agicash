...
function makeArgs(
  ...
  return { command: 'pay', positional, flags: { pretty: false, ...flags } };
}
...
describe('pay validation', () => {
  ...
  test('rejects missing invoice', async () => {
    const r = await handlePayCommand(makeArgs(), emptyCtx);
    expect(r.action).toBe('error');
    expect(r.code).toBe('MISSING_INVOICE');
  });
  test('rejects invalid invoice', async () => {
    const r = await handlePayCommand(
      makeArgs([], { bolt11: 'notaninvoice' }),
      emptyCtx,
    );
    expect(r.action).toBe('error');
    expect(r.code).toBe('INVALID_INVOICE');
  });
  test('accepts invoice as positional arg', async () => {
    const r = await handlePayCommand(makeArgs(['lnbc100n1invalid']), emptyCtx);
    expect(r.code).not.toBe('MISSING_INVOICE');
    expect(r.code).not.toBe('INVALID_INVOICE');
  });
  ...
  test('rejects when specified account not found', async () => {
    ...
    const r = await handlePayCommand(
      makeArgs([], { bolt11: 'lnbc100n1test', account: 'x' }),
      ctx,
    );
    expect(r.action).toBe('error');
    expect(r.code).toBe('NO_ACCOUNT');
