...
```
Note: `bunx` auto-downloads packages (no `-y` flag needed) and is faster than `npx` in Bun projects.
...
2. **Prefer bunx over npx in Bun projects** (faster, no `-y` flag needed)
...
```json
{
  "local-db": {
    "command": "bunx",
    "args": ["mcp-server-sqlite", "./data.db"]
  },
  "cloud-api": {
    "type": "sse",
    "url": "https://mcp.example.com/sse"
...
Note: Use `bunx` in Bun projects, `npx -y` for Node.js projects.
