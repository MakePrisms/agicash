...
- `references/README.md` - NUT overview and implementation status
...
Use `scripts/parse_nut.py` to extract specific sections:

```bash
python scripts/parse_nut.py references/00.md "Blind Diffie-Hellmann key exchange"
```
...
cp nuts/*.md /path/to/.claude/skills/cashu-protocol/references/
```
