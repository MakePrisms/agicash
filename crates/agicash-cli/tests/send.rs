...
//! token is handed to `agicash receive token` to fund the guest account;
//! then `agicash send token` produces a token a fresh guest can claim back
//! via `agicash receive token`.
//!
//! Run:
...
mod gated {
    ...
    /// `agicash send token 100` with no proofs in the account → exit nonzero,
    ...
    /// `agicash send token 100 --dry-run` after a 200-sat deposit prints a
    /// quote without persisting; a follow-up `agicash send token 100` still
    /// succeeds.
    #[test]
