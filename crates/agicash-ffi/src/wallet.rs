...
use agicash_cashu::{
    ...
    CompleteMintQuoteOutcome, CompleteOutcome, MintQuoteError, ParsedToken, ReceiveSwapError,
...
use agicash_storage_supabase::{
    SupabaseCashuMintQuoteStorage, SupabaseCashuReceiveSwapStorage, SupabaseStorage,
...
impl AgicashWallet {
    ...
    pub async fn start_mint_quote(
        ...
        let currency_enum = Currency::from_str(&currency_str).map_err(|_| {
            FfiError::internal(format!("unsupported currency: {currency_str}"))
        })?;
    ...
    pub async fn complete_mint_quote(
        ...
        Ok(receive_result_from_mint_quote_outcome(outcome, &account, &quote))
    }
}
...
mod tests {
    ...
    fn pick_cashu_for_lightning_returns_only_cashu_btc() {
        ...
        let picked = pick_cashu_account_for_lightning(&accounts, None, Currency::Btc)
            .expect("found");
    ...
    fn pick_cashu_for_lightning_errors_when_ambiguous() {
        ...
        assert!(
            matches!(err, FfiError::Internal { ref message } if message.contains("multiple"))
        );
    }
