...
import {
  type CashuSwapReceiveData,
  CashuSwapReceiveDataSchema,
} from '../transactions/cashu-swap-receive-data';
...
export class CashuTokenSwapRepository {
  ...
  async create(
    ...
    const receiveData: CashuSwapReceiveData = {
  ...
  async toTokenSwap(data: AgicashDbCashuTokenSwap): Promise<CashuTokenSwap> {
    ...
    const receiveData = CashuSwapReceiveDataSchema.parse(decryptedData);
    ...
    return CashuTokenSwapSchema.parse({
      ...
      receiveAmount: receiveData.amountReceived,
    ...
    } satisfies AllUnionFieldsRequired<z.input<typeof CashuTokenSwapSchema>>);
  }
}
