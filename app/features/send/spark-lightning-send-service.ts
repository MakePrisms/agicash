...
type GetSparkLightningSendQuoteOptions = {
  ...
  amount?: Money;
  /**
   * An optional function to get the current exchange rate so that the invoice amount can be converted to the user's preferred currency.
   */
  ...
  async getLightningSendQuote({
    ...
    if (invoice.amountMsat) {
      ...
      if (amount.currency === 'BTC') {
        ...
        // TODO: see if we need this. Our send-store already picks the currency based on the account.
        // We do a similar conversion for cashu send quotes so I did it here, but SPark accounts can only be BTC anyway.
    ...
    return {
      paymentRequest,
      amountRequested: amount ?? (amountRequestedInBtc as Money),
