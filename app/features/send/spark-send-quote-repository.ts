...
import {
  ...
  type SparkLightningSendData,
} from '../transactions/spark-lightning-send-data';
...
export class SparkSendQuoteRepository {
  ...
  async create(
    ...
    const sendData: SparkLightningSendData = {
      paymentRequest,
  ...
  async toQuote(data: AgicashDbSparkSendQuote): Promise<SparkSendQuote> {
    ...
    return SparkSendQuoteSchema.parse({
      ...
      amount: sendData.amountToReceive,
    ...
    } satisfies AllUnionFieldsRequired<z.input<typeof SparkSendQuoteSchema>>);
  }
}
