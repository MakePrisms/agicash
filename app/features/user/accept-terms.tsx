...
export function AcceptTerms({
  ...
  onBack,
  loading,
  requireWalletTerms = true,
  requireGiftCardMintTerms = false,
}: AcceptTermsProps) {
  const [walletTermsAccepted, setWalletTermsAccepted] = useState(false);
  const [giftCardMintTermsAccepted, setGiftCardMintTermsAccepted] =
    useState(false);
  const location = useLocation();

  const allAccepted =
    (!requireWalletTerms || walletTermsAccepted) &&
    (!requireGiftCardMintTerms || giftCardMintTermsAccepted);
            ...
            <div className="flex items-start gap-2">
              <Checkbox
                ...
                onCheckedChange={(value) =>
                  setWalletTermsAccepted(value === true)
                }
          ...
          {requireGiftCardMintTerms && (
            <div className="flex items-start gap-2">
              <Checkbox
                ...
                checked={giftCardMintTermsAccepted}
                onCheckedChange={(value) =>
                  setGiftCardMintTermsAccepted(value === true)
                }
                className="mt-0.5 border-primary-foreground"
              />
              <Label
